    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length >= 1) {
            if (!sender.hasPermission("aura.nofall.others")) {
                msg.send(sender, "common.cannot-toggle-others", "feature", msg.get("toggles.feature-nofall"));
                return true;
            }
        ...
        msg.send(target, "toggles.nofall-self", "state", state);
        if (!target.equals(sender)) {
            msg.send(sender, "toggles.nofall-other", "state", state, "player", target.getName());
        }
        return true;
    }
}
