...
Install **AuraUtils-1.3.0-paper-folia.jar** in `plugins/`, then restart.
...
Replace the jar with `AuraUtils-1.3.0-paper-folia.jar`. After first start, translate `plugins/AuraUtils/lang/en.yml` or add another `lang/<code>.yml`. Add `language:`, `homes.default-limit`, and `update-checker.enabled` to an existing `config.yml` if those keys are missing.
